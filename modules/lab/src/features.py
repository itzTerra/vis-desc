...
@dataclass
class ExtCtx(persistent.Persistent):
...
class FeatureExtractorPipeline:
    ...
    def __init__(self):
        ...
        self._char_features_map = {
            ...
            for i, ngram in enumerate(
                pd.read_csv(DATA_DIR / "features" / "char_ngrams_features.csv")["ngram"]
            )
        }
        self._pos_features_map = {
            ast.literal_eval(ngram): i
            for i, ngram in enumerate(
                pd.read_csv(DATA_DIR / "features" / "pos_ngrams_features.csv")["ngram"]
            )
        }

        node_features = [
            ast.literal_eval(ngram)
            for ngram in pd.read_csv(
        ...
        concreteness_data_single = pd.read_csv(IMAG_DATA_DIR / "words.csv")
        concreteness_data_multi = pd.read_csv(IMAG_DATA_DIR / "multiword.csv")
        ...
        prep_imag_data = pd.read_csv(
            IMAG_DATA_DIR / "prepositions.csv", na_values=["NA"]
        )
        ...
        places_data = (IMAG_DATA_DIR / "places.txt").read_text().splitlines()
    ...
    def extract_ngram_word_length_by_char(self, ctx: ExtCtx) -> NDArray[np.float32]:
        ...
        total_ngrams = min(len(ctx.words) - N + 1, 1)
