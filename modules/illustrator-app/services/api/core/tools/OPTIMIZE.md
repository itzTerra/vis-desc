...
- [x] **Load the smallest viable spaCy model and disable all unused pipeline components.**
  ...
  *Audit result: `en_core_web_sm` already used. `tok2vec` cannot be disabled (tagger + parser both depend on it). Added `"senter"` to disable list (redundant — parser already sets sentence boundaries).*

- [x] **Set `MALLOC_TRIM_THRESHOLD_=131072` and/or preload `jemalloc`/`mimalloc`.**
  ...
  *Added to all env templates (.local and .production).*
...
- [ ] **Configure gunicorn/daphne with `--max-requests` to recycle workers periodically.**
  ...
  *Not applicable: app uses daphne (ASGI), which does not support `--max-requests`. Would need supervisord or an external process manager to recycle the worker periodically.*

---

## `_get_from_doc` / `extract_from_memory`

- [x] **[BREAKING] Drop `ExtractionContext.full_text` — stop collecting raw page text entirely.**
...
- [x] **Replace `normalized_full_text += normalized_line` with a list joined once; track length with a running counter.**
...
- [x] **Stream `cleaned_page_text` directly into a `StringIO` instead of accumulating in a `pages` list.**
...
- [x] **Delete `page_text` immediately after `clean_text` returns.**
...
- [x] **Skip cross-page join regex checks when `pages` is empty (first page only).**
...
- [x] Verify: **`io.StringIO` + `str(full_text)` accumulation.** If the [BREAKING] `full_text` drop
...
- [x] **Pre-compile the `_normalize` regex at class level.**
...
- [x] **Expand the `str.maketrans` table to delete dash variants directly, simplifying the normalize regex.**
  Map `\u2010` (‐), `\u2011` (‑), `\u2012` (‒), `\u2013` (–), `\u2014` (—) → `None` in
...
- [x] **Compile `metadata_keywords` into a single regex.**
...
- [x] **Replace the `text_keywords` set check with a compiled character-class regex.**
...
- [x] **Replace `[kw for kw in self.metadata_keywords if kw in line_lower]` list comprehension with a `sum()` generator.**
...
- [x] **Replace `words_lower = line_lower.split()` with `line_lower.count(" ") + 1` for the word count.**
...
- [x] **Reorder the nine-pattern `any()` generator to front-load cheapest and most-frequently-matching patterns.**
...
- [x] **Reorder `is_end_of_paragraph` to check `line[-1] in self.end_of_sentence_chars` first.**
...
- [x] **Replace `len(prev_line.split()) > 5` with an early-exit counter.**
  `str.split()` allocates a full word list. Use a manual counter that breaks at 6.

- [x] **Cache `line.isupper()` as a local variable.**
...
- [x] **Maintain a rolling `prev_line` variable instead of indexing `lines[i - 1]`.**
...
- [x] **Pre-strip all lines in a single pass before entering the loop.**
...
- [x] Verify: **Word count without `.lower().split()`** — the RAM item replacing `split()` with
  `count(" ") + 1` already avoids both the `.lower()` allocation and the word-list allocation.
  No separate action needed.

- [x] Verify: **Metadata keyword list comprehension** — addressed by the RAM `sum()` item above.
...
- [x] **Inline `_after_clean` into `_sequential_clean`.**
...
- [x] **[BREAKING] Merge the two PDF parsing passes into one.**
...
- [x] **[BREAKING] Remove `ExtractionContext.pdf_bytes`.**
...
- [x] **Replace per-word `{"text": wtext, "rect": (x0, y0, x1, y1)}` dicts with plain 4-tuples `(x0, y0, x1, y1)`.**
...
- [x] **Delete `seg_to_page_to_lines` immediately after the first loop completes.**
...
- [x] **Avoid building `concat_segments = "".join(segments)` — use direct segment indexing.**
...
- [x] **Normalize segments lazily one at a time, not all up front.**
...
- [x] **Delete each page's `words` list at the end of its loop body.**
...
- [x] **Replace triple-chained `.setdefault()` per word with cached intermediate dict references.**
...
- [x] **Replace the f-string `f"{block}.{line_no}"` dict key with a tuple `(block, line_no)`.**
...
- [x] **Pre-compute `y_top` sort keys before calling `sorted()`.**
...
- [x] **Compute `y_top` and `y_bottom` in a single pass.**
...
- [x] **Replace `for w in wlist: ...; break` loops with direct index access.**
...
- [x] **Cache page width/height before the post-processing loop.**
...
- [x] **Compute polygon centroid, min/max, and scaled coordinates in a single pass or with NumPy.**
...
- [x] Verify: **PDF reopened in `align_segments_with_pages`.** Once the [BREAKING] two-pass merge
...
- [ ] Verify: **Single `get_textpage()` call per page.** Confirm that after the two-pass merge,
  ...
  *Note: `_segments_to_page_polygons` still calls `page.get_textpage().extractWORDS()` once per page (for word coordinates). `_get_from_doc` calls it once per page for text. Both share the same open `doc` object now. Pre-extracting word lists in `_get_from_doc` to eliminate the second `get_textpage()` call is a future optimization.*

---

## `_word_overlaps_removed_ranges`

- [x] **Replace the hand-rolled binary search with `bisect.bisect_left`.**
...
- [x] **Replace the post-bisect linear scan with a single index check.**
...
- [x] **Avoid `boundary[:] = deduped` by building the compacted result in-place.**
...
- [x] **[BREAKING] Delete `ctx` explicitly after `align_segments_with_pages` returns.**
...
- [x] **[BREAKING] Free `segments` after the Redis write, before HTTP serialization.**
...
- [x] Verify: **`TextSegmenter` is a module-level singleton, not re-instantiated per request.**
