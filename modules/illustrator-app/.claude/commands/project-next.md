...
4. If there are uncompleted tasks, Use the `project-implementor` subagent to implement the next atomic chunk of tasks from a project:
   ...
   - Run related static checks using `format-lint` skill and tests using `run-tests` skill to verify quality
...
5. Launch 3-4 `code-reviewer` subagents (in parallel if possible), each with a specific focus. Pass the project folder path so reviewers check against project specs:
    ...
    - **Test Reviewer**: Focus exclusively on test coverage. Domain work (queries, mutations, serializers) must have unit tests. Route changes should have Playwright E2E tests. Are tests present, correct, and covering edge cases?"
