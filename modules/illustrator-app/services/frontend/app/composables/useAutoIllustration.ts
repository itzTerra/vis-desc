...
export function useAutoIllustration(opts: {
  ...
  // track which highlights have been enhanced/generated by auto flows
  ...
  */
  function runSelectionOnce(): number[] {
    ...
    console.log(`[AutoIllustration] Selected ${selected.length} highlights (minScore=${minScoreVal}, minGap=${minGap.toFixed(2)}, maxGap=${maxGap.toFixed(2)})`);
    return selected;
  }
  ...
  function clearAutoSelections() {
    ...
    // clear enhanced/generated trackers
    enhancedIds.value.clear();
    generatedIds.value.clear();
  }

  function markEnhanced(ids: number[] = []) {
    for (const id of ids) enhancedIds.value.add(id);
  }

  function markGenerated(ids: number[] = []) {
  ...
  return {
    ...
    // helpers to update derived state from external events
    markEnhanced,
    markGenerated,
    getMaxPossible,
